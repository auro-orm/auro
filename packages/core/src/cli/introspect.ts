...
function generateClientClass(interfaces: Table[], result: string[]) {
  const actionsString = interfaces.map(({ table_name }) => createQuery(snakeToCamel(table_name), table_name)).join('\n\n');

  result.push(createAuroClient(actionsString));
}

async function createJoinForTables(tables: Table[], result: string[]) {
  ...
  if (allJoins.length === 0) {
    result.push(`export const Joins = {} as any;\n\nexport type Connections<T> = never;\n`);
    return;
  }

  const joins = generateJoinsAndConnectionsFromSQLResult(JSON.stringify(allJoins));
  result.push(joins);
}

function generateJoinsAndConnectionsFromSQLResult(data: string): string {
  ...
  const importStatements = generateImportStatements(joins);
...
function generateImportStatements(joins: Joins): string {
  const importStatements = [];

  for (const table in joins) {
    if (joins.hasOwnProperty(table)) {
      const joinInfo = joins[table];
      importStatements.push(createImport(capitalizeFirstLetter(joinInfo[0].table), joinInfo[0].table));
      importStatements.push(createImport(capitalizeFirstLetter(joinInfo[0].joiningTable), joinInfo[0].joiningTable));
    }
  }
...
export const introspectDatabase = async () => {
  ...
  await createJoinForTables(tables, lines);
